...
if (process.env.NODE_ENV === "production") {
  ...
  console.log("bar")
  const tesselBoard = 'https://stark-tesselator.local:8888/leds';
}
...
router.put('/:id', function (req, res, tesselBoard) {
    if (req.body.toggle === true) {
      const index = req.body.index;
      request(`${tesselBoard}/leds/${index}`, function(err, response, body) {
